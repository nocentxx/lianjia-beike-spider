...
class ErShouBeishuAnalyzer(object):
    def __init__(self):
    ...
    def data_analyzer(self):
        ...
        self.write_house_details()
        self.write_summary()

    def write_house_details(self):
        csv_file = self.summary_path + "/{0}_house_details.csv".format(self.city_pinyin_name)
        print(csv_file)
        if not os.path.isfile(csv_file):
            with open(csv_file, "w") as f:
                f.write(ErShou.excel_title) # write first row(title) of csv file
                for dist, area in self.district_areas_house_info_dict.items():
                    if type(area) is dict:
                        for area, houses in area.items():
                            if len(houses) > 0 and type(houses) is list:
                                for ershou in houses:
                                    f.write(ershou.text())
